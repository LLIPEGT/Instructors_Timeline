...
describe("roomListService",  () => {
    ...
    it("Room should be listed", async () => {
        const adm = {
        ...
        await roomCreateService.execute(room);
        await roomCreateService.execute(room2);

        const listRooms = await roomListService.execute()

        expect(listRooms).toEqual(expect.arrayContaining(listRooms));
    })
})